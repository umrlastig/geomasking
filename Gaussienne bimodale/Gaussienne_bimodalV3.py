...
def kano(xa,ya,xb,yb):
    ...
    cursorK=conn.cursor()
    queryK="Select count(*) from public.bano7893 as b WHERE ST_DWithin(b.geom,ST_SetSRID(ST_Point(%s,%s),2154),ST_Distance(ST_SetSRID(ST_Point(%s,%s),2154),ST_SetSRID(ST_Point(%s,%s),2154)))" 
...
for k in range(1,len(listdataCSV)):
    ...
    #si l'un des deux a une k-anonymité supérieure à 5, on l'ajoute la base anonymisée
    if kano(float(listdataCSV[k][9])+x,float(listdataCSV[k][10])+y,float(listdataCSV[k][9]),float(listdataCSV[k][10]))>=5:
        inter.append(float(listdataCSV[k][9])+x)
        inter.append(float(listdataCSV[k][10])+y)
    elif kano(float(listdataCSV[k][9])-x,float(listdataCSV[k][10])-y,float(listdataCSV[k][9]),float(listdataCSV[k][10]))>=5:
        inter.append(float(listdataCSV[k][9])-x)
        inter.append(float(listdataCSV[k][10])-y)
    else:
        #sinon par 3 fois, on va ajouter 5m dans la direction et dans la direction opposée, jusqu'à obtenir une k-anonymité supérieure à 5 ou arriver aux 3 itérations
        x+=5*math.cos(angle)
        y+=5*math.sin(angle)
        if kano(float(listdataCSV[k][9])+x,float(listdataCSV[k][10])+y,float(listdataCSV[k][9]),float(listdataCSV[k][10]))>=5:
            inter.append(float(listdataCSV[k][9])+x)
...
with open('donnees_anonymisees_bimodal78kmin.csv', 'w', newline='') as file:
